...
class PPM():
    ...
    def needMoney(self):
        ...
        if(self.nextDay != 0):
            ...
            self.money = PPM.checkNeedMoney((self.startTime.weekday()),int(math.ceil((tmp-self.startTime).seconds/(60*30))))
            for i in range(self.nextDay - 1):
                self.money += PPM.checkNeedMoney(tmp.weekday(),48)
                tmp = DT.strptime((tmp+datetime.timedelta(days=1)).date().strftime('%Y-%m-%d'),'%Y-%m-%d')
            self.money += PPM.checkNeedMoney(self.endTime.weekday(),int(math.ceil((self.endTime-tmp).seconds/(60*30))))
        else:
            self.money = PPM.checkNeedMoney((self.startTime.weekday()),int(math.ceil((self.endTime-self.startTime).seconds/(60*30))))
        return self.money
    ...
    def check(self):
        ...
        if(self.nextDay == 0):
            self.printStr += f'共停了 {int(math.ceil((self.endTime - self.startTime).seconds/60))} 分鐘\n'
        elif(self.nextDay == 1):
            ...
            self.printStr += f'第一天共停了 {int(math.ceil((d1 - self.startTime).seconds/60))} 分鐘，第二天共停了 {int(math.ceil((self.endTime -d2).seconds/60))} 分鐘\n'
        else:
            d1 = DT.strptime((self.startTime+datetime.timedelta(days=1)).date().strftime('%Y-%m-%d'),'%Y-%m-%d')
            d2 = DT.strptime((self.endTime).date().strftime('%Y-%m-%d'),'%Y-%m-%d')
            self.printStr += f'第一天共停了 {int(math.ceil((d1 - self.startTime).seconds/60))} 分鐘，最後一天共停了 {int(math.ceil((self.endTime -d2).seconds/60))} 分鐘，並且在這段期間共停了 {self.nextDay - 1} 天\n'
        print(self.printStr)
    #測試用按鈕 無條件進位
    def testCheck(self):
        self.needMoney()
...
if __name__ == '__main__':
    os.system('cls')
    my = PPM()
    my.setStartTime('2023-05-24 23:59:59')
